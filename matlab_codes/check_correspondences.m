...
if q == 3
    initialize(pointTracker,[X1 Y1],I1)
end
if q >3 
    step(pointTracker,I1)
end
...
T1 = eye(4); T2 = eye(4);
T1(1,4) = X(q+segment-1);
T1(2,4) = 0;
T1(3,4) = Z(q+segment-1);
th = P(q+segment-1);
dcm = angle2dcm( Yaw(q+segment-1), P(q+segment-1), R(q+segment-1) )
T1(1:3,1:3) = dcm';
%%
T2 = eye(4);
T2(1,4) = X(q+segment);
...
T2(3,4) = Z(q+segment);
th = P(q+segment);
dcm = angle2dcm( Yaw(q+segment), P(q+segment), R(q+segment) )
T2(1:3,1:3) = dcm';

T2_1 = T2\T1
R2_1 = T2_1(1:3,1:3)
t2_1 = T2_1(1:3,4)/T2_1(4,4)
temp = R2_1'*t2_1
temp_mat = [0, -temp(3), temp(2); temp(3), 0, -temp(1); -temp(2) ,temp(1), 0]
Funda = K'\R2_1*temp_mat*(K\eye(3))
temp_error=diag([X2';Y2';ones(1,size(X2,1))]'*Funda*[X1';Y1';ones(1,size(X1,1))]);
size(temp_error)
 XX1=X1(abs(temp_error) < 0.100001) ;
  YY1=Y1(abs(temp_error) < 0.100001) ;
   XX2=X2(abs(temp_error) < 0.100001) ;
    YY2=Y2(abs(temp_error) < 0.100001) ;
...
% indd =abs(x1(2,:)-1.65) <= 22 & abs(x1(3,:))<= 150 & abs(x2(3,:))<= 150;
