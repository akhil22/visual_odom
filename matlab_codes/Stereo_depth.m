...
Y2=Y2(in);
  ...
  %%
  feature_file = strcat('/home/akhil/Desktop/visual_odom/',seq,'/right_left_deep_match_features/',int2str(q-2+segment-1),'.txt')
...
Y2=Y2(in);
  figure;
  disparity_manual = X1-X2;
  showMatchedFeatures(I1,I3,[X1 Y1],[X2 Y2]);
  mss = size(X1,1);
  for t = 1:mss;
% for t = 1:1;
pix_row = Y1(t);
pix_col = X1(t);
x1(3,t) = fx*base_length./disparity_manual(t);
x1(2,t) = (pix_row-195)*x1(3,1)/fx;
x1(1,t) = (pix_col-672)*x1(3,1)/fx;
% pix_row = X2(t);
% pix_col = Y2(t);
% x2(3,t) = fx*base_length./disparity_manual(t);
...
x1
%%
 feature_file = strcat('/home/akhil/Desktop/visual_odom/',seq,'/right_left_deep_match_features/',int2str(q-2+segment),'.txt')
...
Y2=Y2(in);
  figure;
  disparity_manual = X1-X2;
  showMatchedFeatures(I2,I4,[X1 Y1],[X2 Y2]);
  mss = size(X1,1);
  for t = 1:mss;
% for t = 1:1;
pix_row = Y1(t);
