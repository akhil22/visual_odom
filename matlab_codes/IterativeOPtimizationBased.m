...
pix_row = X1(2,t);
pix_col = X1(1,t);
x1(3,t) = fx*base_length/D1_l(round(pix_col),round(pix_row));
 x1(2,t) = (pix_row-195)*x1(3,1)/fx;
x1(1,t) = (pix_col-672)*x1(3,t)/fx;
pix_row = X2(2,t);
pix_col = X2(1,t);
x2(3,t) = fx*base_length/D2_l(round(pix_col),round(pix_row));
 x2(2,t) = (pix_row-195)*x2(3,1)/fx;
x2(1,t) = (pix_col-672)*x2(3,t)/fx;  
 end
...
t2mp = cov(x1(1,:),x2(3,:));
Sx1z2 = t2mp(1,2);
%    theta(n_model) = atan2(Sz1x2-Sx1z2,Sx1x2+Sz1z2);
calculated_theta(q-2) = theta(n_model);
Rcomputed =[cos(theta(n_model)) 0 sin(theta(n_model)) ; 0 1 0 ; -sin(theta(n_model)) 0 cos(theta(n_model))]';
% pix_row = X2(2,ind);,
% pix_col = X2(1,ind);
...
%%Rcomputed = rotation of 1st frame with respect to second
%%Tresult21= translation of first frame with respect to second
for zz = 1:5
x1xmean = mean(x1(1,:))
x1zmean = mean(x1(3,:))
x2xmean = mean(x2(1,:))
x2zmean = mean(x2(3,:))
Tresult21 = zeros(3,1);
Tresult21(3) = x2zmean - (-x1xmean*sin(-theta(n_model))+x1zmean*cos(theta(n_model)));
Tresult21(1) = x2xmean - (x1xmean*cos(theta(n_model))+x1zmean*sin(-theta(n_model)));
Tresult = -Rcomputed'*Tresult21
  %Tresult=mean(t)*tcapz
  if Tresult(3) > 2
%         break
  end
Tcomputed = [Rcomputed' Tresult];
 ...
 T = T*Tcomputed;
 %%changin for iterative optimization
  ttx = [x2;ones(1,size(x2,2))];
  ttx = Tcomputed*ttx
  x2 = ttx(1:3,:);
  t2mp = cov(x1(1,:),x2(1,:));
Sx1x2 = t2mp(1,2);
...
% theta(n_model) = -atan2(Sx1x2+Sz1z2,Sz1x2-Sx1z2);
% theta(n_model)*180/pi
% theta(n_model) = -real_theta(q-2)+theta(n_model)
theta(n_model) = 0;
Rcomputed =[cos(theta(n_model)) 0 sin(theta(n_model)) ; 0 1 0 ; -sin(theta(n_model)) 0 cos(theta(n_model))]';
% theta(n_model) = real_theta(q-2);
%    Rcomputed = eye(3,3);
