...
pix_row = X1(2,t);
pix_col = X1(1,t);
x1(2,t) = fx*base_length/D1_l(floor(pix_col),floor(pix_row));
% x1(2,t) = (pix_row-195)*x1(3,1)/fx;
x1(1,t) = (pix_col-672)*x1(2,t)/fx;
pix_row = X2(2,t);
pix_col = X2(1,t);
x2(2,t) = fx*base_length/D2_l(floor(pix_col),floor(pix_row));
% x2(2,t) = (pix_row-195)*x2(3,1)/fx;
x2(1,t) = (pix_col-672)*x2(2,t)/fx;  
end
x1xmean = mean(x1(1,:));
x1zmean = mean(x1(2,:));
x2xmean = mean(x2(1,:));
...
%t=v(2)/v(1);
Tresult = zeros(3,1);
Tresult(3) = x2zmean - (-x1xmean*sin(-theta(n_model))+x1zmean*cos(theta(n_model)));
Tresult(1) = x2xmean - (x1xmean*cos(theta(n_model))+x1zmean*sin(-theta(n_model)));
